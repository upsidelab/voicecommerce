...
class VueStorefrontApi {
  ...
  async createCart() {
    try {
      const cartResponse = await axios.post(`${this.endpoint}/api/cart/create?token=`)
      return cartResponse.data.result
    } catch (e) {
      throw new ConnectionError("Failed to create a cart", e)
    }
  }
